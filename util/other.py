...
import threading
import time # 자동 대국 시 딜레이를 위해 추가
...
KOMI = 6.5 # 백돌(AI)의 덤
# AI 기권 임계값 (현재 플레이어의 승률이 이 값보다 낮으면 기권합니다)
RESIGN_THRESHOLD = 0.05 
# AI vs AI 모드 시 착수 간 딜레이 (초) - GUI 업데이트 시간을 위해 필요
...
# --- 3. AI 플레이어 로직 함수 (흑돌/백돌 모두 처리하도록 일반화) ---
def make_ai_move_turn(board: GoBoard, model: keras.Model):
    """ AI Player가 현재 턴의 플레이어(흑/백)에 대한 착수를 선택하고 실행합니다. """
    if board.is_game_over:
        return False
        
    player = board.current_player
    player_name = "흑돌" if player == 1 else "백돌"
    
    # 1. 승률 확인 및 기권 결정 (기권 기능 비활성화됨)
    win_prob_black = board.get_win_probability()
    current_player_win_prob = win_prob_black if player == 1 else (1.0 - win_prob_black)
    
    # # --- 이전 AI 기권 로직 (비활성화) ---
    # total_stones = np.sum(board.board != 0) 
    # if total_stones > 8 and current_player_win_prob < RESIGN_THRESHOLD:
    #     print(f"🤖 AI({player_name}) 승률이 {current_player_win_prob*100:.1f}%로 매우 낮아 기권합니다. (총 돌 개수: {total_stones})")
    #     board.resign()
    #     return True 
    ...
    for action_index in sorted_indices:
        ...
        # is_valid_move는 현재 board.current_player 기준으로 검사합니다.
    ...
    if found_valid_move:
        ...
        print(f"🤖 AI({player_name}) 착수: {move_coord}")
        return True
    else:
        # 4. 패스
        print(f"AI({player_name}) 착수 실패: 바둑판에 더 이상 둘 곳이 없습니다. AI가 패스합니다.")
        board.pass_turn()
        return True
...
class GoGUI:
    def __init__(self, master):
        ...
        self.is_ai_vs_ai = False # AI vs AI 모드 상태 플래그
        ...
        # Pass Button
        self.pass_button = tk.Button(self.control_frame, text="Pass", command=self.handle_pass, font=('Arial', 12), bg='lightgray')
        self.pass_button.pack(side=tk.LEFT, padx=5) 
        
        # Resign Button (기권 버튼)
        self.resign_button = tk.Button(self.control_frame, text="Resign (기권)", command=self.handle_resign, font=('Arial', 12), bg='#FF6347', fg='white')
        self.resign_button.pack(side=tk.LEFT, padx=5) 

        # Score Check Button (형세 판단 버튼 추가)
        self.score_check_button = tk.Button(self.control_frame, text="형세 판단 (집 계산)", command=self.handle_score_check, font=('Arial', 12), bg='#4CAF50', fg='white')
        self.score_check_button.pack(side=tk.LEFT, padx=5)

        # 버튼 프레임 2 (AI vs AI)
        self.ai_mode_frame = tk.Frame(master)
        self.ai_mode_frame.pack(pady=5)
        
        # AI vs AI Button
        self.ai_vs_ai_button = tk.Button(self.ai_mode_frame, text="⚫ AI vs AI 자동 대국 ⚪", command=self.handle_ai_vs_ai_start, font=('Arial', 12, 'bold'), bg='#00BFFF', fg='white')
        self.ai_vs_ai_button.pack(side=tk.LEFT, padx=5)
    ...
    def update_status(self):
        ...
        if self.game.is_game_over:
            ...
            self.score_check_button.config(state=tk.DISABLED)
            self.ai_vs_ai_button.config(text="새 게임 시작", command=self.reset_game, state=tk.NORMAL)
            self.is_ai_vs_ai = False # 모드 비활성화
            
            if self.game.winner:
                # 기권으로 인한 종료
                winner_map = {'B': "흑돌", 'W': "백돌"}
                winner_name = winner_map[self.game.winner]
                loser_name = winner_map['W'] if self.game.winner == 'B' else winner_map['B']
                ...
                if black_score > white_score:
                    winner = "AI(흑돌) 승리!" if self.is_ai_vs_ai else "사용자(흑돌) 승리!"
            ...
            player_name = '흑돌' if self.game.current_player == 1 else '백돌'
            
            if self.is_ai_vs_ai:
                 # AI vs AI 모드일 때
                text = (
                    f"AI vs AI 대국 진행 중: {player_name} AI 차례 (연속 패스 {self.game.pass_count}/2)\n"
                    f"⚫ 흑돌 승률: {win_rate_black} | ⚪ 백돌 승률: {win_rate_white}"
                )
            else:
                 # 사용자 vs AI 모드일 때
                player_display = '사용자(흑돌, X)' if self.game.current_player == 1 else 'AI(백돌, O)'
                text = (
                    f"현재 차례: {player_display} (연속 패스 {self.game.pass_count}/2)\n"
                    f"⚫ 흑돌 승률: {win_rate_black} | ⚪ 백돌 승률: {win_rate_white}"
                )
            
        self.status_label.config(text=text)

    def toggle_game_controls(self, state):
        """ 사용자 입력 버튼들을 활성화/비활성화합니다. """
        self.pass_button.config(state=state)
        self.resign_button.config(state=state)
        self.ai_vs_ai_button.config(state=state)
        self.score_check_button.config(state=state)


    def handle_click(self, event):
        """ 사용자의 마우스 클릭을 처리합니다. """
        # AI vs AI 모드이거나 AI 턴이거나 게임이 끝났으면 사용자 클릭 무시
        if self.is_ai_vs_ai or self.game.current_player != 1 or self.game.is_game_over or self.ai_lock.locked():
            return 
    ...
    def handle_score_check(self):
        ...
        result_message = (
            f"--- 현재 형세 판단 (단순 지역 계산) ---\n\n"
            f"⚫ 흑돌 점수: {black_score:.1f} 점\n"
            f"⚪ 백돌 점수 (덤 {KOMI}점 포함): {white_score:.1f} 점\n\n"
        )
    ...
    def handle_ai_vs_ai_start(self):
        """ AI vs AI 자동 대국 모드를 시작합니다. """
        if self.is_ai_vs_ai:
            # 이미 실행 중이면 무시 (버튼 텍스트가 "AI vs AI 자동 대국"일 때만 호출되어야 함)
            return

        if not messagebox.askyesno("AI vs AI 대국 시작", "현재 게임을 초기화하고 AI vs AI 대국을 시작하시겠습니까?"):
            return

        self.reset_game() # 게임 상태 초기화
        self.is_ai_vs_ai = True
        
        self.pass_button.config(state=tk.DISABLED)
        self.resign_button.config(state=tk.DISABLED)
        self.score_check_button.config(state=tk.DISABLED)
        self.ai_vs_ai_button.config(text="AI vs AI 진행 중...", state=tk.DISABLED)
        
        self.update_status()
        self.ai_vs_ai_loop()

    def ai_vs_ai_loop(self):
        """ AI vs AI 대국을 자동으로 진행합니다. """
        if not self.is_ai_vs_ai or self.game.is_game_over:
            self.update_status() # 게임 종료 시 최종 상태 업데이트
            return
            
        # 1. 현재 턴 AI 착수 실행
        with self.ai_lock:
            # make_ai_move_turn는 흑돌(1)과 백돌(2) 모두 처리합니다.
            make_ai_move_turn(self.game, self.policy_model) 

        # 2. GUI 업데이트
        self.draw_board()
        self.update_status()

        # 3. 게임이 아직 끝나지 않았다면, 딜레이 후 다음 턴 예약
        if not self.game.is_game_over:
            self.master.after(AI_VS_AI_DELAY, self.ai_vs_ai_loop)
        else:
            # 게임 종료 시 버튼 상태 업데이트는 update_status에서 처리됨
            pass

    def start_ai_play(self):
        """ 사용자 vs AI 모드에서 AI(백돌) 턴을 시작합니다. """
        if self.game.current_player == 2 and not self.game.is_game_over and not self.is_ai_vs_ai:
            with self.ai_lock:
                # 1. AI Logic: 사용자 vs AI 모드에서는 AI(백돌)만 처리합니다.
                make_ai_move_turn(self.game, self.policy_model)
    ...
    def reset_game(self):
        ...
        self.score_check_button.config(state=tk.NORMAL)
        self.ai_vs_ai_button.config(text="⚫ AI vs AI 자동 대국 ⚪", command=self.handle_ai_vs_ai_start, state=tk.NORMAL)
